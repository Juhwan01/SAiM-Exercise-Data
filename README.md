...
`RPE 자율조절` `주간 볼륨 기준` `세트/렙/강도 처방` `주기화 모델` `실시간 코칭` `운동 선택` `측정/평가` `회복` `회복/디로드` `영양` `보충제` `생리학` `부상 관리` `스트레칭` `카디오` `특수 모집단` `행동 코칭` `기타`
...
```
1. rules/A11.yaml 같은 파일 하나 열기
2. DSL.md §1 "룰 카드 한 장의 모양" 참고
3. 14개 필드가 마스터 CSV의 어느 컬럼에서 왔는지 DSL.md §2에서 확인
```
