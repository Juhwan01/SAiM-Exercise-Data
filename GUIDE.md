...
빠른 개요는 [`README.md`](./README.md)에, 룰 본문은 [`RULEBASE.md`](./RULEBASE.md)에 있습니다.
...
먼저 [`README.md`](./README.md)로 5분 개요 → 본 가이드 [`#3 파일 한 줄 요약표`](#3-파일-한-줄-요약표)에서 어떤 파일이 있는지 → [`RULEBASE.md`](./RULEBASE.md) 첫 100줄로 톤·구조 감 잡기.

### PM / 기획자

**메인**: [`RULEBASE.md`](./RULEBASE.md)
...
| [`RULEBASE.md`](./RULEBASE.md) | 팀 회람용 메인. 5×7 매트릭스 + 모든 룰 인라인 |
| [`DSL.md`](./DSL.md) | 룰 한 장의 표기법 정의 (한국어 평이체) |
| [`GAPS.md`](./GAPS.md) | 자료 빈틈 리포트 (셀별 카운트·신뢰도 분포·미등록 ID) |
| [`rules/{ID}.yaml`](./rules/) | 153개 정형 룰 카드. 마스터 CSV 1:1 변환 결과 |
...
| 파일 | 한 줄 |
|---|---|
| [`.scripts/master_index.json`](./.scripts/master_index.json) | ID → 메타 빠른 조회. RULEBASE 빌드 입력 |
...
매트릭스 + 본문 + 부록 구조:
1. **머리말 + 같이 보는 파일 인덱스** (DSL.md / rules/ / GAPS.md / 마스터 CSV)
2. **5×7 매트릭스** — 한눈에 보는 (목표×모듈) 룰 ID 매핑 표
3. **본문** — 모듈별·목표별 룰 리스트. 각 룰 옆에 `[ID ★신뢰도 - 평이항목명](URL)` 인라인 링크
4. **부록** — 153행 역참조 표
5. **검증 메모** — 매트릭스 등록 카운트·신뢰도 분포·자료 빈틈 명시

마스터 CSV에서 파생된 카탈로그. 직접 편집하지 말고 마스터 행을 수정한 뒤 [`DSL.md`](./DSL.md) 명세를 따라 변환기를 재작성해서 재생성.

### `DSL.md` — 룰 한 장 표기법 정의

룰 카드 한 개가 어떤 형식으로 적히는지 정의한 명세. 마스터 CSV의 14컬럼 + RULEBASE.md 매트릭스 위치를 1:1로 도출한 한국어 평이체 표기법. 새 룰을 만들 때 이 명세에 맞춰 `rules/{ID}.yaml`을 작성하면 됨.

### `rules/{ID}.yaml` — 153개 룰 카드

마스터 CSV 153행을 1:1로 변환한 정형 룰 카드. 앱이 룰을 실행할 때 읽는 단위. 카드 한 장 = 룰 한 개. 본문은 마스터 CSV 행과 자구 일치, 매트릭스 위치는 RULEBASE.md에서 도출.

### `GAPS.md` — 자료 빈틈 리포트

자동 생성 리포트. 어떤 컬럼이 비어 있는지 / 5목표×7모듈 셀별 룰 수 / 신뢰도 분포 / 매트릭스 미등록 ID(`J01` 등)를 사실 그대로 카운트.

### `.scripts/master_index.json`

ID를 키로 한 빠른 메타 조회 JSON. 분류·항목·신뢰도·출처를 즉시 lookup. 룰엔진 캐시 / RULEBASE 빌드 입력.
...
# RULEBASE.md / rules/*.yaml 재생성은 DSL.md 명세 기반 변환기를 새로 작성해야 함
# (기존 build_rulebase_v2.py / build_rules.py 임시 변환기는 작업 종료 후 삭제됨)
...
[`RULEBASE.md`](./RULEBASE.md) 링크를 슬랙/노션에 공유. 첫 화면의 5×7 매트릭스가 그대로 한 장 요약 역할을 합니다.
...
```python
# RULEBASE.md의 5×7 매트릭스를 그대로 코드로:
ROUTING = {
...
goal_*.csv, output/*.tsv, RULEBASE.md, master_index.json은 모두 마스터에서 자동 생성됩니다. **직접 편집하지 마세요.**
...
3. python3 .scripts/build_tsv.py
   → output/sheet_*.tsv 재빌드
        ↓
4. (룰베이스 빌드 단계는 DSL.md 명세 기반 변환기를 새로 작성해야 함.
    기존 임시 변환기는 작업 종료 후 삭제됨)
        ↓
...
- **RULEBASE.md / rules/*.yaml 직접 편집 금지** — 마스터 CSV에서 파생된 자료. 마스터 행을 수정한 뒤 [`DSL.md`](./DSL.md) 명세에 맞춰 변환기를 새로 작성해서 재생성
...
A. 두 가지 방법:
- ID 알면: [`RULEBASE.md`](./RULEBASE.md)에서 Ctrl+F로 ID 검색
...
| **파생 파일** | 마스터에서 자동 생성되는 파일 (goal_*.csv, output/*.tsv, RULEBASE.md) |
...
이 가이드로 답이 안 나오면:
- 1차: [`RULEBASE.md`](./RULEBASE.md) 매트릭스에서 키워드 검색
